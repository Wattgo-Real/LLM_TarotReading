...
class TarotDataGenerator:
    def __init__(self, img_dir="./Data/TarotScanAll", canvas_size=(800, 600)):
        ...
        self.image_paths = [[it for it in self.image_paths_realdata if os.path.basename(it).replace("tarot_env_", "").split("_")[0].split(".")[0] == str(i)] for i in range(79)]
        
        self.class_pool = []
        
        # Load background images for textured backgrounds
        self.bg_dir = r"C:\Users\User\Desktop\TarotTrainingData\ImageNet"
        self.bg_paths = glob.glob(os.path.join(self.bg_dir, "*.jpg"))
        print(f"Loaded {len(self.image_paths)} tarot images and {len(self.bg_paths)} background texture images.")
    ...
    def _apply_perspective(self, card, tilt_x=0.3, tilt_type='top'):
        """
    ...
    def _apply_color_jitter(self, img):
        """Applies random color jitter (brightness, contrast, saturation, sharpness)."""
    ...
    def generate_sample(self):
        # Base background: 20% solid color, 80% textured background
        if random.random() < 0.2 or not self.bg_paths:
            if random.random() > 0.5:
            ...
            try:
                with Image.open(bg_path) as bg_img:
                    bg = bg_img.convert("RGB").resize(self.canvas_size, Image.Resampling.LANCZOS)
            except Exception as e:
                # Fallback to solid background on error
                print(f"Warning: Failed to load background {bg_path}, using fallback solid color: {e}")
                bg = Image.new("RGB", self.canvas_size, (random.randint(40, 70), random.randint(40, 70), random.randint(40, 70)))
        # 1. Determine Global parameters for the whole image
        # Consistent scale for all cards in this image
        global_scale = random.uniform(0.75, 1) 
        ...
        selected_paths = []
        for _ in range(len(active_regions)):
            if not self.class_pool:
                self.class_pool = list(range(79))
                random.shuffle(self.class_pool)
            selected_paths.append(self.class_pool.pop())
        labels = []
        ...
        final_img = self._apply_color_jitter(final_img)
...
if __name__ == "__main__":
    #generator = TarotDataGenerator(img_dir="C:/Users/User/Desktop/TarotTrainingData/TarotScanS2")
    generator = TarotDataGenerator()
    output_dir = "C:/Users/User/Desktop/TarotTrainingData/dataset_train"
    data_num = 20000
